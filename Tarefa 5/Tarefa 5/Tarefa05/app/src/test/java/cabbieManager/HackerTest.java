...
public class HackerTest {
    ...
    /**
     * Test to verify that InvalidPaymentMethodException is thrown when an invalid
     * payment method is provided.
     */
    @Test
    public void testRidePayment_throwsInvalidPaymentMethodException() {
        Exception exception = assertThrows(InvalidPaymentMethodException.class, () -> {
            // Tentativa de criar um pagamento com método de pagamento inválido
            new RidePayment("rideId", LocalDateTime.now(), 10.0f, "InvalidPaymentMethod");
        });

        assertEquals("Invalid payment method: InvalidPaymentMethod", exception.getMessage());
    }
     ...
     * 
     * The method requestRide should throw an IllegalArgumentException when the
     * pickup
     ...
     * The expected error message is "Invalid location name: Aeroporto".
     */
    @Test
    public void testRide_thorwsIllegalArgumentException_InvalidPickupLocation() {
        Ride testRide = new Ride();
        Exception exception = assertThrows(IllegalArgumentException.class, () -> {
            testRide.requestRide("Aeroporta", "Museu");
        });

        assertEquals("Invalid location name: Aeroporta", exception.getMessage());
    ...
    /**
     * Tests if the method update throws a NumberFormatException when the rate is
     * not a valid float.
     * 
     * The method update should throw a NumberFormatException when the rate is not a
     * valid float.
     * This test case tests this by calling the method update with an invalid rate
     * and asserting that a NumberFormatException is thrown.
     * The expected error message is "For input string: 'invalid_rate'".
     */
    @Test
    public void testUpdateCabbie_throwsNumberFormatException_InvalidRate() {
        Cabbie cabbieTest = new Cabbie();
        cabbieTest.register();

        Exception exception = assertThrows(NumberFormatException.class, () -> {
            cabbieTest.update("rate", "invalid_rate"); // Invalid rate
        });

        assertEquals("For input string: \"invalid_rate\"", exception.getMessage());
    }
    ...
    public void testRidPayment_throwsNullPointforStartTime() {
        Exception exception = assertThrows(NullPointerException.class, () -> {
            RidePayment rpTest = new RidePayment("rideId", null, 5.0f, "Dinheiro"); // Invalid phone number with
    ...
    /**
     * Tests if the constructor RidePayment throws an InvalidRideDistanceException
     * when the ride distance is equal to zero.
     * 
     * The constructor RidePayment should throw an InvalidRideDistanceException when
     * the ride distance is equal to zero.
     * This test case tests this by calling the constructor with a ride distance of
     * zero and asserting that an InvalidRideDistanceException is thrown.
